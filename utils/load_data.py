...
def load_acm(ratio):
    ...
    参数:
    ratio: 划分数据集的比例（用于加载不同训练、验证、测试集）
    ...
    path = "./data/acm/"  # 数据存放路径

    label = np.load(path + "labels.npy").astype('int32')  # 加载标签数据并转换为 int32 类型
    label = encode_onehot(label)  # 进行 One-hot 编码

    feat_p = sp.load_npz(path + "p_feat.npz")  # 加载论文特征矩阵（稀疏格式）
    print(feat_p.shape)
    pap = sp.load_npz(path + "pap.npz")  # 加载论文-作者-论文（PAP）邻接矩阵
    psp = sp.load_npz(path + "psp.npz")  # 加载论文-学科-论文（PSP）邻接矩阵
    adjs = [pap, psp]  # 将两个邻接矩阵存入列表
    adjs = [sparse_mx_to_torch_sparse_tensor(adj).coalesce() for adj in adjs]  # 转换为 PyTorch 稀疏张量并进行合并（coalesce）
    ...
    label = th.FloatTensor(label)  # 将标签转换为 PyTorch 浮点张量

    feat_p = th.FloatTensor(preprocess_features(feat_p))  # 预处理论文特征，并转换为 PyTorch 浮点张量

    train = [th.LongTensor(i) for i in train]  # 将训练集索引转换为 PyTorch 长整型张量
    val = [th.LongTensor(i) for i in val]  # 将验证集索引转换为 PyTorch 长整型张量
    test = [th.LongTensor(i) for i in test]  # 将测试集索引转换为 PyTorch 长整型张量

    return feat_p, adjs, label, train, val, test  # 返回处理后的数据


def load_cora(ratio):


    path = "./data/cora/"  # 数据存放路径

    label = np.load(path + "labels.npy").astype('int32')  # 加载标签数据并转换为 int32 类型
    label = encode_onehot(label)  # 进行 One-hot 编码

    feat_p = sp.load_npz(path + "feat.npz")  # 加载论文特征矩阵（稀疏格式）
    print(feat_p.shape)
    adjs = sp.load_npz(path + "adjs.npz")
    adjs = [sparse_mx_to_torch_sparse_tensor(adj).coalesce() for adj in adjs]  # 转换为 PyTorch 稀疏张量并进行合并（coalesce）
